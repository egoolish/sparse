...
fig1 = figure(1);
    ...
    title('Linear Deterministic Case, P = 5');
    hold on;
    legend on;
fig2 = figure(2);
    xlabel('Log N');
    ylabel('Log I_N(\epsilon)');
    title('Linear Random Case, P = 5');
    hold on;
    legend on;
for epsilon=epsilonrange
    ...
    randpoly = polyfit(logNrange(int64(4*end/5):end), lograndVector(int64(4*end/5):end), 1);
...
txt_title = uicontrol('Style', 'text', 'String', ['Linear p = 5 for N=', num2str(max(Nrange))]);
